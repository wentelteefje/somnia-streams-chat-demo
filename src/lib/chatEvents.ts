...
// Ensures the chat event schema (ID + signature) is registered; registers and waits for confirmation if missing
export async function ensureChatEventSchema() {
  ...
  if (!tx || tx instanceof Error) {
    throw new Error(
      `Failed to register event schema: ${
        tx instanceof Error ? tx.message : "null tx"
      }`
    );
  }
  ...
  if (!Array.isArray(after) || after.length === 0) {
    throw new Error(
      "Event schema still not visible after registration confirmation"
    );
  }
}
