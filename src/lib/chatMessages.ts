...
function isNoDataRevert(err: unknown): boolean {
  try {
    ...
    if (Array.isArray(anyErr?.metaMessages))
      parts.push(anyErr.metaMessages.join(" | "));
    if (anyErr?.cause?.message) parts.push(String(anyErr.cause.message));
    if (anyErr?.cause?.shortMessage)
      parts.push(String(anyErr.cause.shortMessage));
...
// React hook that periodically fetches chat messages for a room from Somnia Streams
export function useChatMessages(
  ...
  const loadMessages = useCallback(async () => {
    try {
      ...
      let rows: unknown[] = [];
      try {
        const resp = await sdk.streams.getAllPublisherDataForSchema(
          schemaId,
          publisher
        );
      ...
      for (const row of rows) {
        ...
        newMessages.push({
          timestamp: ms,
          roomId: rid,
          content: String(unwrapValue(row[2]) ?? ""),
